...
def get_navigation(user: User | None) -> tuple[NavGroup, ...]:
    ...
    if has_permission(user, "core.backup.read"):
        admin_items.append(NavItem("Sauvegardes", "/sauvegardes/", "check"))
    if has_permission(user, "core.audit.read"):
        admin_items.append(NavItem("Journal d'audit", "/journal-audit/", "search"))
    if admin_items:
    ...
    if module_is_activated("rh"):
        if has_permission(user, "rh.employee.read"):
            rh_items.append(NavItem("Employés", "/employes/", "user"))
        if has_permission(user, "rh.leave_request.approve"):
            rh_items.append(NavItem("Congés", "/conges-a-traiter/", "check"))
    if rh_items:
